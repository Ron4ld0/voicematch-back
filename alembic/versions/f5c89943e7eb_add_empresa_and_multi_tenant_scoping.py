...
down_revision: Union[str, Sequence[str], None] = 'c3a1b8e92f14'
...
def upgrade() -> None:
    ...
    empresa_table = op.create_table('empresa',
    ...
    sa.Column('configuracoes_json', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    ...
    op.create_foreign_key(None, 'candidatura', 'empresa', ['empresa_id'], ['id'], ondelete='CASCADE')
    op.create_foreign_key(None, 'habilidade', 'empresa', ['empresa_id'], ['id'], ondelete='CASCADE')
    op.create_foreign_key(None, 'recrutador', 'empresa', ['empresa_id'], ['id'], ondelete='CASCADE')
    op.create_foreign_key(None, 'vaga', 'empresa', ['empresa_id'], ['id'], ondelete='CASCADE')
...
def downgrade() -> None:
    ...
    op.add_column('recrutador', sa.Column('empresa', sa.VARCHAR(length=255), autoincrement=False, nullable=True))
    op.execute("UPDATE recrutador SET empresa = 'Empresa Padrão'")
    op.alter_column('recrutador', 'empresa', nullable=False)
    
    op.add_column('recrutador', sa.Column('cnpj', sa.VARCHAR(length=14), autoincrement=False, nullable=True))
