...
def get_candidato_by_email(
    db: Session, email: str, empresa_id: UUID
) -> Candidato | None:
    return (
        db.query(Candidato)
        .filter(Candidato.email == email, Candidato.empresa_id == empresa_id)
        .first()
    )
...
def get_candidatos(
    ...
    return (
        db.query(Candidato)
        .filter(Candidato.empresa_id == empresa_id)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def create_candidato(
    db: Session, candidato_in: CandidatoCreate, empresa_id: UUID
) -> Candidato:
